...
sendRouter.post('/', requireApiKey, sendLimiter, async (req: Request, res: Response): Promise<void> => {
  const { template_slug, recipient, data, version, alias, from_email, from_name } = req.body;
  ...
  try {
    const result = await sendEmail({ template_slug, recipient, data: data ?? {}, app: req.emailApp!, version, alias, from_email, from_name });
    if (result.success) {
...
sendRouter.post('/raw', requireApiKey, sendLimiter, async (req: Request, res: Response): Promise<void> => {
  const { subject, html, recipient, alias, from_name, from_email } = req.body as {
    subject?: string; html?: string; recipient?: string; alias?: string; from_name?: string; from_email?: string;
  };
  ...
  try {
    const result = await sendRawEmail({ subject, html, recipient, alias, from_name, from_email, app: req.emailApp! });
    if (result.success) {
      res.json({ success: true, messageId: result.messageId });
    } else {
      res.status(500).json({ success: false, error: result.error });
